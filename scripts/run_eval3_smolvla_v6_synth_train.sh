#!/usr/bin/env bash
# Eval3 SmolVLA v6 launcher for the new synthetic ChArUco-derived datasets.
#
# Trains on the 9 dataset_v3_synth_<celeb>_<position>_2 datasets produced by
# tools/eval3_synth_dataset_gen.py. Same env-var contract and image-augmentation
# defaults as v5 — only difference is the dataset list (synth instead of v2 raw)
# and a larger default batch (H100 80 GiB has the VRAM headroom).
#
# Prereqs:
#   EVAL3_INSTALL_SMOLVLA_DEPS=1 ./install.sh
#   (or: uv pip install transformers accelerate sentencepiece num2words)
#
# Smoke (~5 min):
#   EVAL3_TRAIN_STEPS=200 EVAL3_BATCH=2 EVAL3_POLICY_DEVICE=cuda \
#     ./scripts/run_eval3_smolvla_v6_synth_train.sh
#
# Full Brev run:
#   tmux new -s eval3_v6
#   mkdir -p outputs/train/logs
#   EVAL3_POLICY_DEVICE=cuda ./scripts/run_eval3_smolvla_v6_synth_train.sh 2>&1 \
#     | tee outputs/train/logs/eval3_3way_60k_v6_synth.log
#
# Knobs (all optional, defaults shown):
#   EVAL3_TRAIN_STEPS=60000   EVAL3_BATCH=16        EVAL3_POLICY_DEVICE=cuda
#   EVAL3_PEAK_LR=1e-4        EVAL3_DECAY_LR=1e-6   EVAL3_WARMUP_STEPS=1000
#   EVAL3_DECAY_STEPS=59000   EVAL3_SAVE_FREQ=10000
#   EVAL3_RESUME_FROM=lerobot/smolvla_base   # or a Hub/local checkpoint path
#   EVAL3_TRAIN_OUT=outputs/train/eval3_3way_60k_v6_synth
#   EVAL3_JOB_NAME=eval3_3way_60k_v6_synth
...
REPO="RobotLearningVLA/dataset_v3_synth_taylor_swift_left_2"
EXTRA_REPOS="RobotLearningVLA/dataset_v3_synth_taylor_swift_middle_2,RobotLearningVLA/dataset_v3_synth_taylor_swift_right_2,RobotLearningVLA/dataset_v3_synth_barack_obama_left_2,RobotLearningVLA/dataset_v3_synth_barack_obama_middle_2,RobotLearningVLA/dataset_v3_synth_barack_obama_right_2,RobotLearningVLA/dataset_v3_synth_yann_lecun_left_2,RobotLearningVLA/dataset_v3_synth_yann_lecun_middle_2,RobotLearningVLA/dataset_v3_synth_yann_lecun_right_2"

OUT="${EVAL3_TRAIN_OUT:-outputs/train/eval3_3way_60k_v6_synth}"
JOB="${EVAL3_JOB_NAME:-eval3_3way_60k_v6_synth}"
STEPS="${EVAL3_TRAIN_STEPS:-60000}"
BATCH="${EVAL3_BATCH:-16}"
...
echo ">> Eval3 v6 synthetic-corpus train"
